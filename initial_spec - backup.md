...
* If only tracking data is available and the tracking data is over water plot the line solid blue and animate a boat icon along the tracking line
* If only tracking data is available and the tracking data is over land plot the line solid blue and animate a person icon along the tracking line
* If sailing data is available, animate a blue line graphic arrow moving from the start to the end of the tracks. The arrow should point towards the twa and the length of the arrow is proportional to the tws
* If sailing data is available, animate a green line graphic arrow moving from the start to the end of the tracks. The arrow should point towards the cog and the length of the arrow is proportional to the sog
* If sailing data is available, animate an orange line graphic arrow moving from the start to the end of the tracks. The arrow should point towards the awa and the length of the arrow is proportional to the aws
* If sailing data is available, use a consistent scaling for all arrow lengths
* On the right of the screen add a window with three tabs for Home, Photo and Annotation
* If sailing data is available, on the home tab add a small legend explaining the arrows and place a radio button in front of each entry to allow the user to select displaying the arrow or not. 
* On the home tab add an entry in current data to show distance travelled in nautical miles and kilometers
* If sailing data is available, on the home tab add text explaining the line colors
* If sailing data is available, on the home tab display the values for date/time, lat, lon, rpm, cog, sog, twa, tws, awa and aws
* If only tracking data is available and the track is over land, calculate the sog and cog from the gps coordinate data as it varies over time and display cog as bearing and sog as speed in km/hr
* If only tracking data is available, on the home tab display the values for date/time, lat, lon, sog and cog. add next to cog and sog (est.)
...
* when loading photos only load pictures taken between the start and end timestamp in the tracking data
